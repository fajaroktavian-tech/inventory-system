                ...
                <flux:table.row :key="$student->id">
                    <flux:table.cell>
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="font-semibold text-zinc-800 dark:text-white leading-tight">{{ $student->name }}
                                </p>
                    ...
                    {{-- Tambahkan Badge Status di kolom NIS atau Profil --}}
                    <flux:table.cell>
                        <div class="space-y-1">
                            ...
                            {{-- Badge Status --}}
                            <div>
                                <flux:badge color="{{ $student->is_active ? 'green' : 'zinc' }}" size="sm">
                    ...
                    <flux:table.cell>
                        <div class="flex justify gap-2">
                            {{-- Tombol Detail Baru --}}
                            <flux:button variant="ghost" size="sm" icon="eye" wire:click="showDetail({{ $student->id }})">
                                Detail</flux:button>
                            ...
                            <flux:button size="sm" variant="ghost"
                                icon="{{ $student->is_active ? 'lock-closed' : 'lock-open' }}"
                                wire:click="toggleStatus({{ $student->id }})"
                                wire:confirm="Yakin ingin mengubah status siswa ini?">
                ...
                <div class="flex flex-col items-center">
                    ...
                    <div class="relative">
                        @if($selectedStudent->avatar)
                            <img src="{{ asset('storage/' . $selectedStudent->avatar) }}" alt="{{ $selectedStudent->name }}"
                                class="w-32 h-32 object-cover rounded-3xl shadow-lg border-4 border-white dark:border-zinc-800">
                        @else
                            {{-- Placeholder jika tidak ada foto --}}
                            <div
                                class="w-48 h-48 flex items-center justify-center bg-zinc-100 dark:bg-zinc-800 rounded-3xl border-4 border-white dark:border-zinc-700">
                ...
                <div class="grid grid-cols-2 gap-x-4 gap-y-5 text-sm">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="font-semibold text-zinc-800 dark:text-white">
                            {{ $selectedStudent->class?->prodi?->name ?? '-' }}
                        </p>
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="font-semibold text-zinc-800 dark:text-white">{{ $selectedStudent->class?->name ?? '-' }}
                        </p>
                    </div>
                    ...
                    <div class="col-span-2 space-y-1">
                        ...
                        <p class="font-semibold text-zinc-800 dark:text-white leading-relaxed">
